...
test('Vercel function accepts a valid mobile translation request',async()=>{
 ...
 const handler=createHandler({translateImpl:async(body,received)=>{captured=body;options=received;return {demo:false,versions:[]};}});
 const req={method:'POST',headers:{'x-forwarded-for':'203.0.113.8','x-vercel-oidc-token':'runtime-oidc'},body:{scene:'love',mode:'rewrite',mine:'ENFP',theirs:'INTP',text:'你为什么总是不回我的消息？'}};
 ...
 assert.equal(options.env.VERCEL_OIDC_TOKEN,'runtime-oidc');
});
