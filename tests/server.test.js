...
test('HTTP demo endpoint and malformed JSON',()=>withServer({env:{DEMO_MODE:'true'}},async base=>{
 let r=await fetch(base+'/api/translate',{method:'POST',body:JSON.stringify({...input,text:'我这周很累，周末不想一起出去。'})});assert.equal(r.status,200);assert.equal((await r.json()).demo,true);
 r=await fetch(base+'/api/translate',{method:'POST',body:JSON.stringify(input)});assert.equal(r.status,503);assert.match((await r.json()).error,/公开演示.*自定义内容/);
